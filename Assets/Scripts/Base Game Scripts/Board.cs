...
public class Board : MonoBehaviour
{
        ...
        for (int i = 0; i < width; i++)
        {
            for (int j = 0; j < height; j++)
            {
                if (!blankSpaces[i, j] && !concreteTiles[i, j] && !slimeTiles[i, j])
                {
                    ...
                    GameObject backgrounTile = Instantiate(tilePrefab, tilePosition, Quaternion.identity) as GameObject;
    ...
    private MatchType ColumnOrRow()
    {
        ...
        //Cycle throught all of match Copy and decide if a bomb needs to be made
    ...
    {
        //How many objects are in findMatches currentMateches?
            ...
            for(int j = 0; j < height; j ++)
            {
                ...
                if(!blankSpaces[i, j] && allDots[i, j] == null && !concreteTiles[i, j] && !slimeTiles[i, j])
                {
                    ...
                    for(int k = j + 1; k < height; k ++)
                    {
                        ...
                        if(allDots[i, k]!= null)
                        {
                            //move that dot to this empy space
    ...
    private void CheckToMakeSlime()
    {
        ...
        for(int i = 0; i < width; i ++)
        {
            for(int j = 0; j < height; j++)
            {
                ...
                {
                    //Call another method to make new slime
                    MakeNewSlime();
                    return;
                }
            }
